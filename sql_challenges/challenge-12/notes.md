...
- Aggregate task counts and percent of total by status.
	#### SOLUTION:
	```sql
	SELECT status,
	       COUNT(*) AS task_count,
...
- Join teams, users, and tasks and count tasks per team.
	#### SOLUTION:
	```sql
...
- Count open, in-progress, and blocked tasks per user.
	#### SOLUTION:
	```sql
	SELECT u.full_name,
	       t.name AS team_name,
...
- Filter overdue, non-completed tasks and show owner/team.
	#### SOLUTION:
	```sql
	SELECT ts.title,
	       ts.status,
...
- Count task statuses by priority.
	#### SOLUTION:
	```sql
	SELECT priority,
...
- Add a line chart for completed tasks grouped by day.
	#### SOLUTION:
	```sql
	SELECT TRUNC(completed_at) AS completed_date,
