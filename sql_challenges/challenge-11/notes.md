...
- Add a `Comment` ORM model with required fields and relationships.
	...
	2. `Task` should have `comments` to navigate task threads.
	3. Use `ondelete="CASCADE"` and `cascade="all, delete-orphan"` so comments are removed with a task.
...
- Autogenerate and inspect a migration for the `comments` table.
	...
	2. `downgrade()` reverts schema changes (drops `comments`).
	3. Downgrading removes the table and any data stored in it.
...
- Use ORM-only CRUD to create a team, user, tasks, update, and delete.
	...
	with Session(engine) as session:
		...
		session.add_all([team, user] + tasks)
		session.commit()
		...
		lowest = session.query(Task).filter(Task.description == "priority: low").one()
...
- Roll back the last migration and note what happens.
	...
	```
	Questions:
	1. The column or table from the last migration is removed.
	2. Any data stored in that column or table is lost after rollback.

#### Exercise 5 — Concept Check

- Answer the conceptual questions briefly.
	#### SOLUTION:
	1. ORM vs raw SQL: ORM gives reusable models, relationships, and DB-agnostic queries; raw SQL is more manual for complex apps.
	2. Migrations: They track schema changes and keep environments consistent.
	...
	5. Relationships: They simplify joins and navigation between related rows.
